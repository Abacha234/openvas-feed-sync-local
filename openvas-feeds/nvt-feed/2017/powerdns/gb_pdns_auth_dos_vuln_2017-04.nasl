...
if(description)
{
  ...
  script_name("PowerDNS Authoritative Server <= 4.0.4 DoS Vulnerability (2017-04)");
  ...
  script_tag(name:"insight", value:"An issue has been found in the API component of PowerDNS
  Authoritative Server, where some operations that have an impact on the state of the server are
  still allowed even though the API has been configured as read-only via the api-readonly keyword.
  This missing check allows an attacker with valid API credentials could flush the cache, trigger a
  zone transfer or send a NOTIFY.");

  script_tag(name:"impact", value:"A remote attacker may cause a partial DoS condition.");

  script_tag(name:"affected", value:"PowerDNS Authoritative Server version 4.0.4 and prior.");
