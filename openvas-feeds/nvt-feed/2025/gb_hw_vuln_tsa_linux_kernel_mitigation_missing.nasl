...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.119211");
  ...
  script_name("Missing Linux Kernel mitigations for 'TSA' hardware vulnerabilities (AMD-SB-7029)");
  ...
  Linux Kernel side for the referenced 'AMD Transient Scheduler Attacks' (TSA) hardware
  vulnerabilities.");
  ...
  - CVE-2024-36348: A transient execution vulnerability may allow a user process to infer the
  control registers speculatively even if UMIP[3] feature is enabled, potentially resulting in
  information leakage.

  - CVE-2024-36349: A transient execution vulnerability may allow a user process to infer TSC_AUX
  even when such a read is disabled, potentially resulting in information leakage.

  - CVE-2024-36350: A transient execution vulnerability may allow an attacker to infer data from
  previous stores, potentially resulting in the leakage of privileged information.

  - CVE-2024-36357: A transient execution vulnerability may allow an attacker to infer data in the
  L1D cache, potentially resulting in the leakage of sensitive information across privileged
  boundaries.");
...
# nb: Only AMD CPUs seems to be affected. But we're only checking this if the sysfs file is
