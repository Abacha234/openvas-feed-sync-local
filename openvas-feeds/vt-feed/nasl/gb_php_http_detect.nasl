...
foreach checkFile( checkFiles ) {
  ...
  if( "PHP" >< banner ) {
    ...
    if( ! isnull( version ) && version != "" ) {
      conclUrl = http_report_vuln_url( port:port, url:checkFile, url_only:TRUE );
      break;
    }
  }
}
...
if( version || phpSessId ) {
  ...
  set_kb_item( name:"php/http/port", value:port );
  set_kb_item( name:"php/http/" + port + "/installs", value:port + "#---#" + location + "#---#" + version + "#---#" + concluded + "#---#" + conclUrl );
}
