...
if(description)
{
  ...
  script_xref(name:"URL", value:"https://www.akamai.com/blog/security-research/2024-php-exploit-cve-one-day-after-disclosure");
  script_xref(name:"URL", value:"https://cybersecuritynews.com/php-vulnerability-exploited/");
...
# nb: The "hello.world" and "test.hello" are mentioned on various blog posts and probed by some
# botnets. It is currently unclear if these are really live products which had used these so the
# URLs are added here just to be sure...
if( ! isnull( _phps ) ) {
  _phps = make_list( "/", "/index.php", "/hello.world", "/test.hello", _phps );
} else {
  _phps = make_list( "/", "/index.php", "/hello.world", "/test.hello" );
}
...
# - Our "_phps" list has (currently) at least 12 entries so we use a little bit more then this as
...
# - Some of the URLs are using upper/lowercase URL encoding, these have been used like this by
#   various Botnets and have been added below for wider detection coverage (e.g. some basic WAF
#   could be bypassed by these)
#
...
#
# Seen as used by XMRig
post_urls[i++] = "%add+allow_url_include%3d1+%add+auto_prepend_file%3dphp://input";
#
# Seen as used by Gh0st RAT
post_urls[i++] = "%ADd+allow_url_include%3D1+%ADd+auto_prepend_file%3Dphp://input";
#
# Seen as used by Muhstik malware
post_urls[i++] = "%ADd+allow_url_include%3D1+-d+auto_prepend_file%3Dphp://input";
