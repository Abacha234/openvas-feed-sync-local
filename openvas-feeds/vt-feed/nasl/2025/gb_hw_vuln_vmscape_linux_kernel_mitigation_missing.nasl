...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.119212");
  ...
  script_name("Missing Linux Kernel mitigations for 'VMScape' hardware vulnerability");
  ...
  script_xref(name:"URL", value:"https://www.kernel.org/doc/html/latest/admin-guide/hw-vuln/vmscape.html");
  script_xref(name:"URL", value:"https://comsec.ethz.ch/research/microarch/vmscape-exposing-and-exploiting-incomplete-branch-predictor-isolation-in-cloud-environments/");

  script_tag(name:"summary", value:"The remote host is missing one or more known mitigation(s) on
  Linux Kernel side for the referenced 'VMScape' hardware vulnerability.");
  ...
  script_tag(name:"insight", value:"VMScape is a vulnerability that may allow a guest to influence
  the branch prediction in host userspace.

  Even if a hypervisor may not have any sensitive data like disk encryption keys, guest-userspace
  may be able to attack the guest-kernel using the hypervisor as a confused deputy.");

  script_tag(name:"affected", value:"Various Intel and AMD CPUs. Please see the references for the
  full list of affected CPUs. It particularly affects hypervisors like QEMU.");
...
covered_vuln = "vmscape";

if( ! mitigation_status = get_kb_item( "ssh/hw_vulns/kernel_mitigations/missing_or_vulnerable/" + covered_vuln ) )
  exit( 99 );

# nb: Only Intel and AMD CPUs seems to be affected. But we're only checking this if the sysfs file is
