...
if( login_banner && login_banner != "None Available" ) {
  ...
  else if( "Vega Session Controller" >< login_banner ) {
  ...
  # Nexus 3000 Switch
    ...
    os_register_and_report( os:"Cisco NX-OS", cpe:"cpe:/o:cisco:nx-os", banner_type:BANNER_TYPE, port:port, banner:login_banner, desc:SCRIPT_DESC, runs_key:"unixoide" );
    exit( 0 );
  }

  # e.g.:
  # Welcome to EMC UnityVSA 12GB
  # Welcome to EMC Unity 500 64GB
  else if( concl = egrep( pattern:"Welcome to EMC Unity", string:login_banner ) ) {
    os_register_and_report( os:"Linux", cpe:"cpe:/o:linux:kernel", banner_type:BANNER_TYPE, port:port, banner:chomp( concl ), desc:SCRIPT_DESC, runs_key:"unixoide" );
    exit( 0 );
  }
}
...
# nb: For banners see e.g. https://github.com/BetterCrypto/Applied-Crypto-Hardening/blob/master/unsorted/ssh/ssh_version_strings.txt

# nb:
# - Order matters, as some banners can include several keywords
# - For newer versions see e.g. https://packages.ubuntu.com/search?keywords=openssh-server
if( "ubuntu" >< tolower( banner ) ) {
  ...
  if( "SSH-2.0-OpenSSH_9.7p1 Ubuntu-7ubuntu4.3" >< banner ) {
    os_register_and_report( os:"Ubuntu", version:"24.10", cpe:"cpe:/o:canonical:ubuntu_linux", banner_type:BANNER_TYPE, port:port, banner:banner, desc:SCRIPT_DESC, runs_key:"unixoide" );
    exit( 0 );
  }

  if( "SSH-2.0-OpenSSH_9.9p1 Ubuntu-3ubuntu3.1" >< banner ) {
    os_register_and_report( os:"Ubuntu", version:"25.04", cpe:"cpe:/o:canonical:ubuntu_linux", banner_type:BANNER_TYPE, port:port, banner:banner, desc:SCRIPT_DESC, runs_key:"unixoide" );
    exit( 0 );
  }

  # nb:
  # - https://packages.ubuntu.com/questing/openssh-server is showing "1:10.0p1-5ubuntu5" but live
  #   system had used the first so both had been added for now...
  # - The third "hpn" seems to be some kind of "High Performance" build like e.g.:
  #   SSH-2.0-OpenSSH_10.0p2_hpn18.7.1 Ubuntu-1ubuntu5
  #   SSH-2.0-OpenSSH_10.0p1_hpn18.7.0 Ubuntu-1ubuntu7
  if( "SSH-2.0-OpenSSH_10.0p2 Ubuntu-5ubuntu5" >< banner || "SSH-2.0-OpenSSH_10.0p1 Ubuntu-5ubuntu5" || ( "SSH-2.0-OpenSSH_10.0p" >< banner && " Ubuntu-1ubuntu" >< banner ) ) {
    os_register_and_report( os:"Ubuntu", version:"questing", cpe:"cpe:/o:canonical:ubuntu_linux", banner_type:BANNER_TYPE, port:port, banner:banner, desc:SCRIPT_DESC, runs_key:"unixoide" );
    exit( 0 );
  }
...
# For newer / current supported versions of Debian see e.g. the "News" section here:
# https://tracker.debian.org/pkg/openssh
  ...
  # nb: https://packages.ubuntu.com/questing/openssh-server is showing "1:10.0p1-7" but live system
  # had used the first so both had been added for now...
  # SSH-2.0-OpenSSH_10.0p2 Debian-7
  if( "SSH-2.0-OpenSSH_10.0p2 Debian-7" >< banner || "SSH-2.0-OpenSSH_10.0p1 Debian-7" >< banner || ( "~bpo13" >< banner && "SSH-2.0-OpenSSH_" >< banner ) ) {
    os_register_and_report( os:"Debian GNU/Linux", version:"13", cpe:"cpe:/o:debian:debian_linux", banner_type:BANNER_TYPE, port:port, banner:banner, desc:SCRIPT_DESC, runs_key:"unixoide" );
    exit( 0 );
  }

  # SSH-2.0-OpenSSH_10.2p1 Debian
  if( "SSH-2.0-OpenSSH_10.2p1 Debian" >< banner ) {
    os_register_and_report( os:"Debian GNU/Linux", version:"forky/sid", cpe:"cpe:/o:debian:debian_linux:forky%2Fsid", banner_type:BANNER_TYPE, port:port, banner:banner, desc:SCRIPT_DESC, runs_key:"unixoide", full_cpe:TRUE );
    exit( 0 );
  }
...
#
# nb: More detailed OS Detection covered in gb_greenbone_os_ssh_login_detect.nasl
