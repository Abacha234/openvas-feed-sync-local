...
if(description)
{
  ...
  script_name("Zabbix Server/Agent Detection (TCP)");
  ...
  script_tag(name:"summary", value:"TCP based detection of the ZABBIX Server/Agent.");
...
foreach port( ports ) {
  foreach req( reqs ) {
    ...
    if( buf =~ "^ZBXD" ) {
      ...
      set_kb_item( name:"zabbix/server_or_agent/detected", value:TRUE );
      set_kb_item( name:"zabbix/server_or_agent/tcp/detected", value:TRUE );
