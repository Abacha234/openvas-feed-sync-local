...
if (description)
{
  ...
  script_name("PHP 5.3.x < 5.3.27 Heap Buffer Overflow Vulnerability (Jul 2013) - Windows");
  ...
  script_tag(name:"insight", value:"The flaw is triggered as user-supplied input is not properly
  validated when handling malformed XML input.");

  script_tag(name:"impact", value:"Successful exploitation will allow attackers to cause a
  heap-based buffer overflow, resulting in a denial of service or potentially allowing the execution
  of arbitrary code.");

  script_tag(name:"affected", value:"PHP prior to version 5.3.27.");

  script_tag(name:"solution", value:"Update to version 5.3.27 or later.");
...
if (version_in_range(version: version, test_version: "5.3.0", test_version2: "5.3.26")) {
  report = report_fixed_ver(installed_version: version, fixed_version: "5.3.27", install_path: location);
