...
foreach proto( make_list( "udp", "tcp" ) ) {
  ...
  foreach key( keys( banners ) ) {
    ...
    # 9.18.41-1~deb12u1-Debian (for bind9 package 1:9.18.41-1~deb12u1)
    # 9.20.11-4~bpo12+1-Debian (for bind9 package 1:9.20.11-4~bpo12+1 which was a backport)
    #
    ...
    #
    # 9.20.11-4-Debian (for bind9 package 1:9.20.11-4 which was the first release on Debian 13)
    # 9.20.15-1~deb13u1-Debian (for bind9 package 1:9.20.15-1~deb13u1)
    #
    ...
    if( "-Debian" >< banner || ( "PowerDNS Authoritative Server" >< banner && "debian.org)" >< banner ) ) {
      ...
      else if( "9.20.11-4-Debian" >< banner || "~deb13" >< banner || "+deb13" >< banner || "~bpo13" >< banner ) {
        os_register_and_report( os:"Debian GNU/Linux", version:"13", cpe:"cpe:/o:debian:debian_linux", banner_type:BANNER_TYPE, port:port, proto:proto, banner:banner, desc:SCRIPT_DESC, runs_key:"unixoide" );
      }
