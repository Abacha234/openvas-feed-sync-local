...
if(description)
{
  ...
  script_name("Adobe Commerce / Magento Detection (HTTP)");
  ...
  script_tag(name:"summary", value:"HTTP based detection of Adobe Commerce / Magento.");
...
foreach dir( make_list_unique( "/", "/magento", "/shop", http_cgi_dirs( port:port ) ) ) {
  ...
  if( res2 && res2 =~ "^HTTP/1\.[01] 200" &&
      ( "/skin/frontend/" >< res2 ||
        "text/x-magento-init" >< res2 ||
        " Magento. All rights reserved.<" >< res2 ||
        '<meta name="keywords" content="Magento, Varien, E-commerce"/>' >< res2 ||
        egrep( string:res2, pattern:"^X-Magento-(Cache-Control|Cache-Debug|Tags)\s*:.+", icase:TRUE )
    ) ) {
    found = TRUE;
    if( conclUrl )
      conclUrl += '\n';
  ...
  if( found ) {
    ...
    if( ! flag ) {
      # Magento Connect Manager ver. 1.7.0.1
    ...
    if( res3 && "magento" >< res3 && "=== Improvements ===" >< res3 ) {
      if( res3 =~ "(c|C)ommunity_(e|E)dition" ) {
        CE    = TRUE;
        extra = "  Edition gathered from " + http_report_vuln_url( port:port, url:url3, url_only:TRUE );
      }
    ...
    # <p class="text-version">Version 2.4.7-p6</p>
    # <p class="text-version">Version 2.4.8-p2</p>
    ...
    if( ! EE || ! CE || ! B2B ) {
      ...
      if( res5 && res5 =~ "(M|m)agento (E|e)nterprise (E|e)dition" && res5 =~ "license.*enterprise.edition" ) {
        EE    = TRUE;
        extra = "  Edition gathered from " + http_report_vuln_url( port:port, url:url5, url_only:TRUE );
      } else {
        ...
        if( res6 && res6 =~ "(M|m)agento" && res6 =~ "license.*opensource.*Free" ) {
          CE    = TRUE;
          extra = "  Edition gathered from " + http_report_vuln_url( port:port, url:url6, url_only:TRUE );
        }
      }
    }
