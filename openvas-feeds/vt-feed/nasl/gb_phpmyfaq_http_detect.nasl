...
if(description)
{
  ...
  script_xref(name:"URL", value:"https://www.phpmyfaq.de/");
...
foreach dir(make_list_unique("/faq", "/phpmyfaq", http_cgi_dirs(port: port))) {
  ...
  if (egrep(pattern: "powered by phpMyFAQ", string: buf, icase: TRUE)) {
    ...
    set_kb_item(name: "phpmyfaq/http/detected", value: TRUE);

    cpe = build_cpe(value: vers, exp: "^([0-9.]+)", base: "cpe:/a:phpmyfaq:phpmyfaq:");
    if (!cpe)
