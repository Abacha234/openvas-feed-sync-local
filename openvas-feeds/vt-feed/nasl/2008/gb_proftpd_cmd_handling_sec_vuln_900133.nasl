...
if(description)
{
  ...
  script_name("ProFTPD < 1.3.2rc3 CSRF Vulnerability");
  ...
  script_tag(name:"summary", value:"ProFTPD Server is prone to a cross-site request forgery (CSRF)
  vulnerability.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"The flaw exists due to the application truncating an overly
  long FTP command, and improperly interpreting the remainder string as a new FTP command.");

  script_tag(name:"impact", value:"This can be exploited to execute arbitrary FTP commands on
  another user's session privileges.");

  script_tag(name:"affected", value:"ProFTPD Server prior to version 1.3.2rc3.");
