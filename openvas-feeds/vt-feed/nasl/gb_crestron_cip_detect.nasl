...
if (description)
{
  ...
  script_require_udp_ports("Services/udp/unknown", 41794);
...
port = unknownservice_get_port(default: 41794, ipproto: "udp");

host = get_host_name();

# nb:
# - Source port has to be 41794 or the non-default port as well
...
if (hexstr(recv[0]) != "15" || strlen(recv) < 266) {
  unknown_banner_set(port: port, banner: recv, ipproto: "udp", set_oid_based: TRUE);
  exit(0);
}
