...
if(description)
{
  ...
  script_xref(name:"URL", value:"https://web.archive.org/web/20210121190838/http://www.securityfocus.com/bid/35029");

  script_tag(name:"summary", value:"NSD (Name Server Daemon) is prone to an off-by-one buffer
  overflow vulnerability because the server fails to perform adequate boundary checks on
  user-supplied data.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"impact", value:"Successfully exploiting this issue will allow attackers to
  execute arbitrary code within the context of the affected server. Failed exploit attempts will
  result in a denial-of-service condition.");

  script_tag(name:"affected", value:"NSD versions prior to 3.2.2.");
