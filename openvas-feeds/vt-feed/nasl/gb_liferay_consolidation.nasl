  ...
  foreach install (install_list) {
    ...
    if (edition =~ "^DXP" || edition =~ "Digital Experience Platform") {
      ...
      if (version =~ "Update") {
        ...
        cpe_version = elements[0] + "update" + elements[2];
        internal_version = elements[0] + "." + elements[2];
      }

      cpe = build_cpe(value: tolower(cpe_version), exp: "([0-9.q]+)(update[0-9]+)?", base: "cpe:/a:liferay:dxp:");
      if (!cpe)
    ...
    register_product(cpe: cpe, location: install, port: port, service: source, internal_version: internal_version);
