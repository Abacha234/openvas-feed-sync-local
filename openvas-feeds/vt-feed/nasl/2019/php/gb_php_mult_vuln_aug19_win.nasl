...
if (description)
{
  ...
  script_tag(name:"summary", value:"PHP is prone to multiple heap-based buffer overflows
  vulnerabilities.");
  ...
  script_tag(name:"affected", value:"PHP version 7.x before 7.1.31, 7.2.x before 7.2.21 and 7.3.x
  before 7.3.8.");
...
if (version_in_range(version: version, test_version: "7.0", test_version2: "7.1.30")) {
  report = report_fixed_ver(installed_version: version, fixed_version: "7.1.31", install_path: location);
  security_message(port: port, data: report);
  exit(0);
}

if (version_in_range(version: version, test_version: "7.2", test_version2: "7.2.20")) {
  report = report_fixed_ver(installed_version: version, fixed_version: "7.2.21", install_path: location);
  security_message(port: port, data: report);
  exit(0);
}

if (version_in_range(version: version, test_version: "7.3", test_version2: "7.3.7")) {
  report = report_fixed_ver(installed_version: version, fixed_version: "7.3.8", install_path: location);
  security_message(port: port, data: report);
  exit(0);
}
