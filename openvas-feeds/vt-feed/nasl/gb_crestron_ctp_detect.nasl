...
# DIN-AP2 Control Console
# AV2 Control Console
# MC3 Console
# CP3 Console
if (recv !~ "((Control|MC3|CP3) Console|Crestron Terminal Protocol Console Opened)") {
...
if (recv && "Bad or Incomplete Command" >!< recv) {
  ...
  mod = eregmatch(pattern: '(Processor Type|Panel ID)\\s*:([^\r]+)', string: recv);
  if (!isnull(mod[2]))
    model = ereg_replace(pattern: '(\t| )', string: mod[2], replace: '');
...
# nb: When the device is an output card or controller, all the info is in VER command
# eg.:
# DMC-STRO - Streaming output card [v1.2911.00043, #86229510] Stream:d2.0
# DIN-DALI-2 Lighting Controller [v3.5168.00001, #911287E7]
if (model == "unknown") {
