...
if(description)
{
  ...
  script_name("SSL/TLS: OpenSSL Padding Oracle Vulnerability (20160503) - Active Check");
  ...
  script_xref(name:"URL", value:"https://openssl-library.org/news/secadv/20160503.txt");
  script_xref(name:"URL", value:"https://www.exploit-db.com/exploits/39768");
  script_xref(name:"URL", value:"https://web-in-security.blogspot.com/2016/05/curious-padding-oracle-in-openssl-cve.html");

  script_tag(name:"summary", value:"OpenSSL is prone to a padding oracle vulnerability.");

  script_tag(name:"vuldetect", value:"Sends an encrypted padded message and checks the returned
  alert (Record Overflow if vulnerable, Bad Record Mac if no vulnerable).");

  script_tag(name:"insight", value:"The vulnerability is due to not considering memory allocation
  during a certain padding check.");

  script_tag(name:"impact", value:"Exploiting this vulnerability allows remote attackers to obtain
  sensitive cleartext information via a padding oracle attack against an AES CBC session.");

  script_tag(name:"affected", value:"OpenSSL versions prior to 1.0.1t and 1.0.2 prior to 1.0.2h.");

  script_tag(name:"solution", value:"Update to version 1.0.1t, 1.0.2h or later.");
