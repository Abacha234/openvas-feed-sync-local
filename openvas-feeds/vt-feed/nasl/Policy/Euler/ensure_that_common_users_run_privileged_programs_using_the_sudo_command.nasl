...
if(description)
{
  ...
  script_add_preference(name:"User", type:"entry", value:"test_sudo", id:1);

  script_xref(name:"Policy", value:"EulerOS Baseline: openEuler Security Configuration Baseline (v1.0.0): 2. Secure Access: 2.4 Access Control: 2.4.5 Ensure That Common Users Run Privileged Programs Using the sudo Command (Requirement)");
  script_xref(name:"Policy", value:"HCE Linux (Huawei Cloud EulerOS): openEuler Security Configuration Baseline (v1.0.0): 2. Secure Access: 2.4 Access Control: 2.4.5 Ensure That Common Users Run Privileged Programs Using the sudo Command (Requirement)");
  script_xref(name:"Policy", value:"openEuler Baseline: openEuler Security Configuration Baseline (v1.0.0): 2. Secure Access: 2.4 Access Control: 2.4.5 Ensure That Common Users Run Privileged Programs Using the sudo Command (Requirement)");
...
user = script_get_preference("User");
...
# grep -E "^'+user+'\\s+ALL=\\(root\\)" /etc/sudoers';
...
# ------------------------------------------------------------------
# CHECK : Check the command `grep -E "^test_sudo\s+ALL=\(root\)" /etc/sudoers`
# ------------------------------------------------------------------

step_cmd = 'grep -E "^'+user+'\\s+ALL=\\(root\\)" /etc/sudoers';
