...
if(description)
{
  ...
  script_xref(name:"Policy", value:"EulerOS Baseline: openEuler Security Configuration Baseline (v1.0.0): 3. Running and Services: 3.3 SSH: 3.3.2 Configure a Proper SSH Service Authentication Mode (Requirement)");
  script_xref(name:"Policy", value:"EulerOS Virtual: openEuler Security Configuration Baseline (v1.0.0): 3. Running and Services: 3.3 SSH: 3.3.2 Configure a Proper SSH Service Authentication Mode (Requirement)");
  script_xref(name:"Policy", value:"openEuler Baseline: openEuler Security Configuration Baseline (v1.0.0): 3. Running and Services: 3.3 SSH: 3.3.2 Configure a Proper SSH Service Authentication Mode (Requirement)");
...
}else if((strstr(actual_value, 'IgnoreRhosts yes') && strstr(actual_value, 'HostbasedAuthentication no')) && (strstr(actual_value, 'PasswordAuthentication yes') || strstr(actual_value, 'ChallengeResponseAuthentication yes') || strstr(actual_value, 'PubkeyAuthentication yes'))){
  compliant = "yes";
