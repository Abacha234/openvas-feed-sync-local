...
if(description)
{
  ...
  script_add_preference(name:"timeout_seconds", type:"entry", value:"300", id:1);

  script_xref(name:"Policy", value:"EulerOS Baseline: openEuler Security Configuration Baseline (v1.0.0): 2. Secure Access: 2.3 Identity Authentication: 2.3.2 Ensure That the Session Timeout Period Is Set Correctly (Requirement)");
  script_xref(name:"Policy", value:"EulerOS Virtual: openEuler Security Configuration Baseline (v1.0.0): 2. Secure Access: 2.3 Identity Authentication: 2.3.2 Ensure That the Session Timeout Period Is Set Correctly (Requirement)");
  script_xref(name:"Policy", value:"HCE Linux (Huawei Cloud EulerOS): openEuler Security Configuration Baseline (v1.0.0): 2. Secure Access: 2.3 Identity Authentication: 2.3.2 Ensure That the Session Timeout Period Is Set Correctly (Requirement)");
  script_xref(name:"Policy", value:"openEuler Baseline: openEuler Security Configuration Baseline (v1.0.0): 2. Secure Access: 2.3 Identity Authentication: 2.3.2 Ensure That the Session Timeout Period Is Set Correctly (Requirement)");
...
timeout_seconds = script_get_preference("timeout_seconds");
...
}else if(actual_value == 'export TMOUT='+timeout_seconds+''){
  compliant = "yes";
