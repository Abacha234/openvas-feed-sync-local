...
if(description)
{
  ...
  script_xref(name:"Policy", value:"EulerOS Baseline: openEuler Security Configuration Baseline (v1.0.0): 4. Log Audit: 4.2 Rsyslog: 4.2.8 Configure the Remote Log Server (Recommendation)");
  script_xref(name:"Policy", value:"EulerOS Virtual: openEuler Security Configuration Baseline (v1.0.0): 4. Log Audit: 4.2 Rsyslog: 4.2.8 Configure the Remote Log Server (Recommendation)");
  script_xref(name:"Policy", value:"openEuler Baseline: openEuler Security Configuration Baseline (v1.0.0): 4. Log Audit: 4.2 Rsyslog: 4.2.8 Configure the Remote Log Server (Recommendation)");
...
command, the symbol . indicates that all logs are displayed on the server. *.* means .__. For
example, mail.info indicates that only mail logs with level info are displayed on the server. @
indicates that the UDP protocol is used. @@ indicates that the TCP protocol is used.
