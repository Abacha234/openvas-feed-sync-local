...
if(description)
{
  ...
  script_xref(name:"Policy", value:"EulerOS Baseline: openEuler Security Configuration Baseline (v1.0.0): 3. Running and Services: 3.5 Kernel: 3.5.12 Disable IP Forwarding (Requirement)");
  script_xref(name:"Policy", value:"EulerOS Virtual: openEuler Security Configuration Baseline (v1.0.0): 3. Running and Services: 3.5 Kernel: 3.5.12 Disable IP Forwarding (Requirement)");
  script_xref(name:"Policy", value:"HCE Linux (Huawei Cloud EulerOS): openEuler Security Configuration Baseline (v1.0.0): 3. Running and Services: 3.5 Kernel: 3.5.12 Disable IP Forwarding (Requirement)");
  script_xref(name:"Policy", value:"openEuler Baseline: openEuler Security Configuration Baseline (v1.0.0): 3. Running and Services: 3.5 Kernel: 3.5.12 Disable IP Forwarding (Requirement)");
...
# grep -Els <quote>^\\s*net\\.ipv4\\.ip_forward\\s*=\\s*1<quote> /etc/sysctl.conf /etc/sysctl.d/*.conf
/usr/lib/sysctl.d/*.conf /run/sysctl.d/*.conf <pipe> while read filename<semicolon> do sed -ri
<quote>s/^\\s*(net\\.ipv4\\.ip_forward\\s*)(=)(\\s*\\S+\\b).*$/# *REMOVED* \\1/<quote> $filename<semicolon>
done<semicolon> sysctl -w net.ipv4.ip_forward=0<semicolon> sysctl -w net.ipv4.route.flush=1

# grep -Els <quote>^\\s*net\\.ipv6\\.conf\\.all\\.forwarding\\s*=\\s*1<quote> /etc/sysctl.conf /etc/sysctl.d/*.conf
/usr/lib/sysctl.d/*.conf /run/sysctl.d/*.conf <pipe> while read filename<semicolon> do sed -ri
<quote>s/^\\s*(net\\.ipv6\\.conf\\.all\\.forwarding\\s*)(=)(\\s*\\S+\\b).*$/# *REMOVED* \\1/<quote> $filename<semicolon>
...
expected_value = '1. The output should contain "net.ipv4.ip_forward = 0"
2. The output should be empty
3. The output should contain "net.ipv6.conf.all.forwarding = 0"
...
step_cmd_check_1 = 'sysctl net.ipv4.ip_forward';
...
if(strstr(step_res_check_1, 'net.ipv4.ip_forward = 0')){
  check_result_1 = TRUE;
}

# ------------------------------------------------------------------
# CHECK 2 :  Verify command `grep -E -s "^\\s*net\\.ipv4\\.ip_forward\\s*=\\s*1" /etc/sysctl.conf /etc/sysctl.d/*.conf /usr/lib/sysctl.d/*.conf /run/sysctl.d/*.conf`
# ------------------------------------------------------------------

step_cmd_check_2 = 'grep -E -s "^\\s*net\\.ipv4\\.ip_forward\\s*=\\s*1" /etc/sysctl.conf /etc/sysctl.d/*.conf /usr/lib/sysctl.d/*.conf /run/sysctl.d/*.conf';
...
# ------------------------------------------------------------------
# CHECK 3 :  Verify command `sysctl net.ipv6.conf.all.forwarding`
# ------------------------------------------------------------------

step_cmd_check_3 = 'sysctl net.ipv6.conf.all.forwarding';
...
if(strstr(step_res_check_3, 'net.ipv6.conf.all.forwarding = 0')){
  check_result_3 = TRUE;
}

# ------------------------------------------------------------------
# CHECK 4 :  Verify command `grep -E -s "^\\s*net\\.ipv6\\.conf\\.all\\.forwarding\\s*=\\s*1" /etc/sysctl.conf /etc/sysctl.d/*.conf /usr/lib/sysctl.d/*.conf /run/sysctl.d/*.conf`
# ------------------------------------------------------------------

step_cmd_check_4 = 'grep -E -s "^\\s*net\\.ipv6\\.conf\\.all\\.forwarding\\s*=\\s*1" /etc/sysctl.conf /etc/sysctl.d/*.conf /usr/lib/sysctl.d/*.conf /run/sysctl.d/*.conf';
...
else if(check_result_1 && check_result_2 && check_result_3 && check_result_4){
  compliant = "yes";
