...
if(description)
{
  ...
  script_mandatory_keys("Compliance/Launch", "policy/ssh/login/eulerosvirtual_openeuler");

  script_xref(name:"Policy", value:"EulerOS Virtual: openEuler Security Configuration Baseline (v1.0.0): 2. Secure Access: 2.4 Access Control: 2.4.1 Restrict the Number of Historical Command Records (Recommendation)");
  script_xref(name:"Policy", value:"openEuler Baseline: openEuler Security Configuration Baseline (v1.0.0): 2. Secure Access: 2.4 Access Control: 2.4.1 Restrict the Number of Historical Command Records (Recommendation)");
...
# grep -qiP <quote>^HISTSIZE<quote> /etc/profile && sed -i <quote>/^HISTSIZE/cHISTSIZE=100<quote> /etc/profile <pipe><pipe>
echo -e <quote>HISTSIZE=100<quote> >> /etc/profile
...
expected_value = '1. The output should be higher than or equal to "1" and be less than or equal to "100"
2. The output should be higher than or equal to "1" and be less than or equal to "100"';
