...
if(description)
{
  ...
  script_xref(name:"Policy", value:"EulerOS Baseline: openEuler Security Configuration Baseline (v1.0.0): 3. Running and Services: 3.2 Firewall: 3.2.8 Configure Proper Policies for INPUT of iptables (Recommendation)");
  script_xref(name:"Policy", value:"openEuler Baseline: openEuler Security Configuration Baseline (v1.0.0): 3. Running and Services: 3.2 Firewall: 3.2.8 Configure Proper Policies for INPUT of iptables (Recommendation)");
...
# iptables -A INPUT -p -s -d --dport -j ACCEPT
...
# ip6tables -A INPUT -p -s -d --dport -j ACCEPT
...
# iptables-save | grep -E "^-A INPUT.+\\<'+protocol+'\\>.+\\<'+port+'\\>.+ACCEPT$"

2. Run the command in the terminal:
# ip6tables-save | grep -E "^-A INPUT.+\\<'+protocol+'\\>.+\\<'+port+'\\>.+ACCEPT$"';
...
step_cmd_check_1 = 'iptables-save | grep -E "^-A INPUT.+\\<'+protocol+'\\>.+\\<'+port+'\\>.+ACCEPT$"';
...
step_cmd_check_2 = 'ip6tables-save | grep -E "^-A INPUT.+\\<'+protocol+'\\>.+\\<'+port+'\\>.+ACCEPT$"';
