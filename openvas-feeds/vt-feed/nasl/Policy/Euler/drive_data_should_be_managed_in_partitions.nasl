...
if(description)
{
  ...
  script_add_preference(name:"CURRENT MOUNTED DIRECTORIES", type:"entry", value:"/boot|/tmp|/home|/var|/usr", id:1);
  script_add_preference(name:"PERMANENT AUTOMOUNT DIRECTORIES", type:"entry", value:"/boot|/tmp|/home|/var|/usr", id:2);

  script_xref(name:"Policy", value:"EulerOS Baseline: openEuler Security Configuration Baseline (v1.0.0): 1. Initial deployment: 1.1 File Systems: 1.1.18 Drive Data Should Be Managed in Partitions (Recommendation)");
  script_xref(name:"Policy", value:"EulerOS Virtual: openEuler Security Configuration Baseline (v1.0.0): 1. Initial deployment: 1.1 File Systems: 1.1.18 Drive Data Should Be Managed in Partitions (Recommendation)");
  script_xref(name:"Policy", value:"HCE Linux (Huawei Cloud EulerOS): openEuler Security Configuration Baseline (v1.0.0): 1. Initial deployment: 1.1 File Systems: 1.1.18 Drive Data Should Be Managed in Partitions (Recommendation)");
  script_xref(name:"Policy", value:"openEuler Baseline: openEuler Security Configuration Baseline (v1.0.0): 1. Initial deployment: 1.1 File Systems: 1.1.18 Drive Data Should Be Managed in Partitions (Recommendation)");
...
current_mounted_directories = script_get_preference("CURRENT MOUNTED DIRECTORIES");
permanent_automount_directories = script_get_preference("PERMANENT AUTOMOUNT DIRECTORIES");
...
# echo <quote>/dev/sdb /home/test ext4 defaults 1 1<quote> >> /etc/fstab";
...
# ------------------------------------------------------------------
# CHECK 1 :  Verify command `df | grep -iE "{{CURRENT MOUNTED DIRECTORIES}}"`
# ------------------------------------------------------------------
