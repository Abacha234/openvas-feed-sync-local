...
if(description)
{
  ...
  script_xref(name:"Policy", value:"EulerOS Baseline: openEuler Security Configuration Baseline (v1.0.0): 4. Log Audit: 4.1 Audit: 4.1.5 Configure Audit Rules for Privilege-Escalated Commands (Recommendation)");
  script_xref(name:"Policy", value:"openEuler Baseline: openEuler Security Configuration Baseline (v1.0.0): 4. Log Audit: 4.1 Audit: 4.1.5 Configure Audit Rules for Privilege-Escalated Commands (Recommendation)");
...
/etc/audit/rules.d/privileged.rules file based on the configured format. In the command, is the
value of UID_MIN in /etc/login.defs, which can be set to 1000 in openEuler.

# find / -xdev -type f \\( -perm -4000 -o -perm -2000 \\) <pipe> awk '{print <quote>-a always,exit -F
path=<quote> $1 <quote> -F perm=x -F auid>= -F auid!=unset -k <quote> }' > /etc/audit/rules.d/privileged.rules
...
}else if(strstr(actual_value, 'not set')){
  compliant = "yes";
