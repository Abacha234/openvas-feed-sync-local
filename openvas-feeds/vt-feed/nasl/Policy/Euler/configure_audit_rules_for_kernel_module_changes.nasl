...
if(description)
{
  ...
  script_xref(name:"Policy", value:"EulerOS Baseline: openEuler Security Configuration Baseline (v1.0.0): 4. Log Audit: 4.1 Audit: 4.1.6 Configure Audit Rules for Kernel Module Changes (Recommendation)");
  script_xref(name:"Policy", value:"openEuler Baseline: openEuler Security Configuration Baseline (v1.0.0): 4. Log Audit: 4.1 Audit: 4.1.6 Configure Audit Rules for Kernel Module Changes (Recommendation)");
...
-w /sbin/insmod -p x -k
-w /sbin/rmmod -p x -k
-w /sbin/modprobe -p x -k
-a always,exit -F arch=b32 -S init_module -S delete_module -k
