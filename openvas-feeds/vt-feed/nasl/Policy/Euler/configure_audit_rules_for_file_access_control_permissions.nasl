...
if(description)
{
  ...
  script_xref(name:"Policy", value:"EulerOS Baseline: openEuler Security Configuration Baseline (v1.0.0): 4. Log Audit: 4.1 Audit: 4.1.18 Configure Audit Rules for File Access Control Permissions (Recommendation)");
  script_xref(name:"Policy", value:"openEuler Baseline: openEuler Security Configuration Baseline (v1.0.0): 4. Log Audit: 4.1 Audit: 4.1.18 Configure Audit Rules for File Access Control Permissions (Recommendation)");
...
/etc/audit/rules.d/ directory, and add audit rules to the file. indicates the value of UID_MIN (the
minimum value of UID when a user is added in the useradd mode) in the /etc/login.defs file. The
default value is 1000 on openEuler.

# vim /etc/audit/rules.d/fileperm.rules
-a always,exit -F arch=b32 -S chmod -S fchmod -S fchmodat -F auid>= -F auid!=unset -k
-a always,exit -F arch=b32 -S chown -S fchown -S fchownat -S lchown -F auid>= -F auid!=unset -k
-a always,exit -F arch=b32 -S setxattr -S lsetxattr -S fsetxattr -S removexattr -S lremovexattr -S
fremovexattr -F auid>= -F auid!=unset -k

For a 64-bit system, add configuration related to arch=b64:

-a always,exit -F arch=b64 -S chmod -S fchmod -S fchmodat -F auid>= -F auid!=unset -k
-a always,exit -F arch=b64 -S chown -S fchown -S fchownat -S lchown -F auid>= -F auid!=unset -k
