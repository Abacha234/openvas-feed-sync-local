...
if(description)
{
  ...
  script_xref(name:"Policy", value:"EulerOS Baseline: openEuler Security Configuration Baseline (v1.0.0): 4. Log Audit: 4.1 Audit: 4.1.2 Ensure That Rotation Is Enabled for Audit Logs (Requirement)");
  script_xref(name:"Policy", value:"openEuler Baseline: openEuler Security Configuration Baseline (v1.0.0): 4. Log Audit: 4.1 Audit: 4.1.2 Ensure That Rotation Is Enabled for Audit Logs (Requirement)");
...
}else if(actual_value =~ 'num_logs\\s*=\\s*([2-9]|[1-9][0-9])' && strstr(actual_value, 'max_log_file_action = ROTATE')){
  compliant = "yes";
