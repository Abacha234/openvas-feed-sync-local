...
# See e.g. this on a current Kernel:
#
# grep ^ /dev/null /sys/devices/system/cpu/vulnerabilities/* | sort
#
# nb: As of 10/2025 no Linux Kernel documentation for the "ghostwrite" and "tsa" mitigations are
# available.
...
foreach known_mitigation( known_mitigations ) {
  ...
  # nb: case insensitive match because there is "Vulnerable" vs. "SMT vulnerable"
    ...
    # nb: Can be used for VTs which should only report if the sysfs interface is there and a
    # "Vulnerable" is reported.
    set_kb_item( name:"ssh/hw_vulns/kernel_mitigations/vulnerable", value:TRUE );
    set_kb_item( name:"ssh/hw_vulns/kernel_mitigations/vulnerable/" + known_mitigation, value:res );

    # nb: Can be used for VTs which should also report if the sysfs interface is missing.
    set_kb_item( name:"ssh/hw_vulns/kernel_mitigations/missing_or_vulnerable", value:TRUE );
    set_kb_item( name:"ssh/hw_vulns/kernel_mitigations/missing_or_vulnerable/" + known_mitigation, value:res );
  }
