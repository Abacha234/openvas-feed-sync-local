...
function check_http_banner( port, banner ) {
  ...
  if( banner = egrep( pattern:"^Server\s*:.*$", string:banner, icase:TRUE ) ) {
    ...
    # - https://packages.qa.debian.org/a/apache2.html ("News" section with entries like "in bookworm-security")
    if( "Apache/" >< banner && "Debian" >< banner ) {
      ...
      # nb: Those have been used Bullseye, Bookworm and Trixie so we're registering all for now with the
      # newest (Trixie) first
      if( "Apache/2.4.65 (Debian)" >< banner ) {
